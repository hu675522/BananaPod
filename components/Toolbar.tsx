    ...
    if (isCropping) {
        return (
            ...
            >
                ...
                <button onClick={onConfirmCrop} className="px-4 py-1.5 text-sm rounded-md bg-green-500 text-white hover:bg-green-600 w-full">{t('toolbar.crop.confirm')}</button>
            </div>
        )
    }
        ...
        >
            ...
            <div className="flex flex-col items-center gap-2 flex-grow">
                ...
                <div className="flex flex-col items-center gap-1">
                    <span className="text-xs text-white">{t('toolbar.strokeColor')}</span>
                    <div className="flex items-center gap-2">
                        <input 
                            ...
                            className="w-8 h-8 p-0 border border-white/30 rounded-md cursor-pointer bg-transparent" 
                        />
                        <div className="flex flex-wrap gap-1 max-w-[60px]">
                            {['#000000', '#ffffff', '#ff0000', '#00ff00', '#0000ff', '#ffff00', '#ff00ff', '#00ffff'].map(color => (
                                <div 
                                    key={color}
                                    onClick={() => setDrawingOptions({ ...drawingOptions, strokeColor: color })}
                                    className="w-4 h-4 rounded-sm cursor-pointer border border-white/30"
                ...
                <div className="flex flex-col items-center gap-1 mt-2">
                    <span className="text-xs text-white">{t('toolbar.strokeWidth')}</span>
                    <div className="flex items-center gap-2">
                        <input 
                            type="range" 
                            min="1" 
                    ...
                    <div className="stroke-width-selector flex gap-1 mt-1">
                        {[2, 5, 10, 20, 30].map(size => (
                            <button 
                                key={size}
                                onClick={() => setDrawingOptions({ ...drawingOptions, strokeWidth: size })}
                                className={`width-option flex items-center justify-center rounded-sm p-1 ${drawingOptions.strokeWidth === size ? 'bg-green-500/50' : 'hover:bg-white/20'}`}
                                title={`${size}px`}
                            >
                                <div 
                                    className="width-dot rounded-full" 
                                    style={{ 
                                        width: Math.min(size, 16), 
                                        height: Math.min(size, 16),
                                        backgroundColor: drawingOptions.strokeWidth === size ? '#ffffff' : '#cccccc',
                                        boxShadow: drawingOptions.strokeWidth === size ? '0 0 3px rgba(255,255,255,0.8)' : 'none'
                                    }}
                                />
                            </button>
                        ))}
                ...
                <div className="w-10 h-px bg-white/30 mt-2"></div>
            ...
            <div className="flex flex-col items-center gap-1 mt-2 z-10">
                ...
                <div className="flex items-center gap-2 z-10">
                    <button 
                        className="p-2 rounded-md transition-colors duration-200 text-white hover:bg-white/20 z-10"
                ...
                <div className="flex gap-1 mt-1 z-10">
                    <button 
                        ...
                        className={`px-2 py-1 text-xs rounded cursor-pointer z-10 text-white ${zoom === 0.5 ? 'bg-green-500' : 'hover:bg-white/10'}`}
                        title="50%"
                    >
                        50%
                    </button>
                    <button 
                        ...
                        className={`px-2 py-1 text-xs rounded cursor-pointer z-10 text-white ${zoom === 1 ? 'bg-green-500' : 'hover:bg-white/10'}`}
                        title="100%"
                    >
                        100%
                    </button>
                    <button 
                        ...
                        className={`px-2 py-1 text-xs rounded cursor-pointer z-10 text-white ${zoom === 2 ? 'bg-green-500' : 'hover:bg-white/10'}`}
                        title="200%"
                    >
                        200%
                    </button>
