        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-40 w-full max-w-3xl px-4">
            ...
            >
                 <div className="flex-shrink-0 flex items-center bg-black/20 rounded-full p-1">
                    <button onClick={() => setGenerationMode('image')} className={`px-3 py-1.5 text-sm rounded-full transition-colors text-white ${generationMode === 'image' ? 'bg-green-500' : 'hover:bg-white/10'}`}>{t('promptBar.imageMode')}</button>
                    <button onClick={() => setGenerationMode('video')} className={`px-3 py-1.5 text-sm rounded-full transition-colors text-white ${generationMode === 'video' ? 'bg-green-500' : 'hover:bg-white/10'}`}>{t('promptBar.videoMode')}</button>
                </div>
                
                {generationMode === 'video' && (
                    <div className="flex-shrink-0 flex items-center bg-black/20 rounded-full p-1 ml-1">
                        <button onClick={() => setVideoAspectRatio('16:9')} title={t('promptBar.aspectRatioHorizontal')} className={`p-1.5 rounded-full transition-colors text-white ${videoAspectRatio === '16:9' ? 'bg-green-500' : 'hover:bg-white/10'}`}>
                        ...
                        <button onClick={() => setVideoAspectRatio('9:16')} title={t('promptBar.aspectRatioVertical')} className={`p-1.5 rounded-full transition-colors text-white ${videoAspectRatio === '9:16' ? 'bg-green-500' : 'hover:bg-white/10'}`}>
