...
export async function editImage(
  ...
  try {
    ...
    if (error instanceof Error) {
        ...
        // 检查常见错误类型
        if (errorMessage.includes('api key') || errorMessage.includes('apikey') || errorMessage.includes('key not valid') || errorMessage.includes('invalid key')) {
            throw new Error('API_KEY_INVALID');
        } else if (errorMessage.includes('network') || errorMessage.includes('timeout') || errorMessage.includes('connection') || errorMessage.includes('fetch')) {
            throw new Error('NETWORK_ERROR');
        } else if (errorMessage.includes('quota') || errorMessage.includes('limit') || errorMessage.includes('rate') || errorMessage.includes('exceeded')) {
            throw new Error('QUOTA_EXCEEDED');
        } else if (errorMessage.includes('permission') || errorMessage.includes('access') || errorMessage.includes('unauthorized')) {
            throw new Error('PERMISSION_DENIED');
        } else if (errorMessage.includes('content') && (errorMessage.includes('policy') || errorMessage.includes('blocked'))) {
